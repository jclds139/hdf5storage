...
def test_file(conv_file_path: Callable, conv_internal_path: Callable, tmp_path: Path):
    ...
    assert isinstance(output, np.ndarray)  # this is both a check for pytest and an assertion for mypy
...
def test_writes_reads(conv_file_path: Callable, conv_internal_path: Callable, tmp_path: Path):
    ...
    assert len(output) == 1
    assert isinstance(output[0], np.ndarray)  # both a check for pytest and an assertion for mypy
...
def test_savemat_loadmat_notv7p3(conv_file_path: Callable, fmt: hdf5storage.MatfileFormat, tmp_path: Path):
